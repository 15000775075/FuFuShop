...
namespace FuFuShop.Admin.Controllers.Financial
{
    ...
    public class CoreCmsBillPaymentsController : ControllerBase
    {
        ...
        // POST: Api/BillPayments/GetPageList
        /// <summary>
        ///     获取列表
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<AdminUiCallBack> GetPageList()
        {
            ...
            var where = PredicateBuilder.True<BillPayments>();
            //获取排序字段
            var orderField = Request.Form["orderField"].FirstOrDefault();
            Expression<Func<BillPayments, object>> orderEx;
            switch (orderField)
            {
                case "paymentId":
            ...
            //获取数据
            var list = await _BillPaymentsServices.QueryPageAsync(where, orderEx, orderBy, pageCurrent,
                pageSize);
            //返回数据
            jm.data = list;
            jm.code = 0;
        ...
        // POST: Api/BillPayments/GetIndex
        /// <summary>
        ///     首页数据
        /// </summary>
        /// <returns></returns>
        ...
        // POST: Api/BillPayments/GetDetails/10
        /// <summary>
        ///     预览数据
        /// </summary>
        ...
        public async Task<AdminUiCallBack> GetDetails([FromBody] FMStringId entity)
        {
            ...
            var model = await _BillPaymentsServices.QueryByClauseAsync(p => p.paymentId == entity.id);
            if (model == null)
            {
                jm.msg = "不存在此信息";
                return jm;
        ...
        // POST: Api/BillPayments/SelectExportExcel/10
        /// <summary>
        ///     选择导出
        /// </summary>
        /// <param name="entity"></param>
        ...
        public async Task<AdminUiCallBack> SelectExportExcel([FromBody] FMArrayStringIds entity)
        {
            ...
            //获取list数据
            var listmodel =
                await _BillPaymentsServices.QueryListByClauseAsync(p => entity.id.Contains(p.paymentId),
            ...
            var fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "-BillPayments导出(选择结果).xls";
        ...
        // POST: Api/BillPayments/QueryExportExcel/10
        /// <summary>
        ///     查询导出
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<AdminUiCallBack> QueryExportExcel()
        {
            ...
            //获取list数据
            var listmodel =
                await _BillPaymentsServices.QueryListByClauseAsync(where, p => p.paymentId, OrderByType.Asc);
            //给sheet1添加第一行的头部标题
            var row1 = sheet1.CreateRow(0);
            row1.CreateCell(0).SetCellValue("支付单号");
            row1.CreateCell(1).SetCellValue("支付金额");
            ...
            var webRootPath = _webHostEnvironment.WebRootPath;
            var tpath = "/files/" + DateTime.Now.ToString("yyyy-MM-dd") + "/";
            var fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "-BillPayments导出(查询结果).xls";
            var filePath = webRootPath + tpath;
            var di = new DirectoryInfo(filePath);
            if (!di.Exists) di.Create();
            var fileHssf = new FileStream(filePath + fileName, FileMode.Create);
