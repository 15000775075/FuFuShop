...
namespace FuFuShop.Admin.Controllers.Financial
{
    ...
    public class BillRefundController : ControllerBase
    {
        ...
        // POST: Api/BillRefund/GetPageList
        /// <summary>
        ///     获取列表
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<AdminUiCallBack> GetPageList()
        {
            ...
            var where = PredicateBuilder.True<BillRefund>();
            //获取排序字段
            var orderField = Request.Form["orderField"].FirstOrDefault();
            Expression<Func<BillRefund, object>> orderEx;
            switch (orderField)
            {
                case "refundId":
            ...
            //获取数据
            var list = await _BillRefundServices.QueryPageAsync(where, orderEx, orderBy, pageCurrent, pageSize);
            //返回数据
            jm.data = list;
            jm.code = 0;
        ...
        // POST: Api/BillRefund/GetDetails/10
        /// <summary>
        ///     预览数据
        /// </summary>
        ...
        public async Task<AdminUiCallBack> GetDetails([FromBody] FMStringId entity)
        {
            ...
            var model = await _BillRefundServices.QueryByIdAsync(entity.id);
            if (model == null)
            {
                jm.msg = "不存在此信息";
                return jm;
        ...
        public async Task<AdminUiCallBack> DoAudit([FromBody] FMDoAuditPost entity)
        {
            ...
            var result =
                await _BillRefundServices.ToRefund(entity.refundId, entity.status, entity.paymentCode);
        ...
        public async Task<AdminUiCallBack> DoReAudit([FromBody] FMStringId entity)
        {
            ...
            var oldModel = await _BillRefundServices.QueryByClauseAsync(p =>
            ...
            var result = await _BillRefundServices.PaymentRefund(entity.id);
        ...
        // POST: Api/BillRefund/SelectExportExcel/10
        /// <summary>
        ///     选择导出
        /// </summary>
        /// <param name="entity"></param>
        ...
        public async Task<AdminUiCallBack> SelectExportExcel([FromBody] FMArrayStringIds entity)
        {
            ...
            //获取list数据
            var listmodel =
                await _BillRefundServices.QueryListByClauseAsync(p => entity.id.Contains(p.refundId),
            ...
            var fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "-BillRefund导出(选择结果).xls";
        ...
        // POST: Api/BillRefund/QueryExportExcel/10
        /// <summary>
        ///     查询导出
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<AdminUiCallBack> QueryExportExcel()
        {
            ...
            //获取list数据
            var listmodel =
                await _BillRefundServices.QueryListByClauseAsync(where, p => p.refundId, OrderByType.Asc);
            //给sheet1添加第一行的头部标题
            var row1 = sheet1.CreateRow(0);
            row1.CreateCell(0).SetCellValue("退款单ID");
            row1.CreateCell(1).SetCellValue("售后单id");
            ...
            var webRootPath = _webHostEnvironment.WebRootPath;
            var tpath = "/files/" + DateTime.Now.ToString("yyyy-MM-dd") + "/";
            var fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "-BillRefund导出(查询结果).xls";
            var filePath = webRootPath + tpath;
            var di = new DirectoryInfo(filePath);
            if (!di.Exists) di.Create();
            var fileHssf = new FileStream(filePath + fileName, FileMode.Create);
