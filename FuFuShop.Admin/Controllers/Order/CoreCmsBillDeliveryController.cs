...
namespace FuFuShop.Admin.Controllers
{
    ...
    public class CoreCmsBillDeliveryController : ControllerBase
    {
        ...
        private readonly IBillDeliveryItemServices _itemServices;
        ...
        // POST: Api/BillDelivery/GetPageList
        /// <summary>
        ///     获取列表
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<AdminUiCallBack> GetPageList()
        {
            ...
            var where = PredicateBuilder.True<BillDelivery>();
            //获取排序字段
            var orderField = Request.Form["orderField"].FirstOrDefault();
            Expression<Func<BillDelivery, object>> orderEx;
            switch (orderField)
            {
                case "deliveryId":
            ...
            //获取数据
            var list = await _BillDeliveryServices.QueryPageAsync(where, orderEx, orderBy, pageCurrent,
                pageSize);
        ...
        // POST: Api/BillDelivery/GetIndex
        /// <summary>
        ///     首页数据
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<AdminUiCallBack> GetEdit([FromBody] FMStringId entity)
        {
            ...
            var model = await _BillDeliveryServices.QueryByIdAsync(entity.id);
            if (model == null)
            {
                jm.msg = "不存在此信息";
                return jm;
        ...
        [HttpPost]
        [Description("编辑提交")]
        public async Task<AdminUiCallBack> DoEdit([FromBody] BillDelivery entity)
        {
            var jm = new AdminUiCallBack();

            var oldModel = await _BillDeliveryServices.QueryByIdAsync(entity.deliveryId);
            if (oldModel == null)
            {
                jm.msg = "不存在此信息";
                return jm;
            ...
            //事物处理过程结束
            var bl = await _BillDeliveryServices.UpdateAsync(oldModel);
            jm.code = bl ? 0 : 1;
        ...
        // POST: Api/BillDelivery/GetDetails/10
        /// <summary>
        ///     预览数据
        /// </summary>
        /// <param name="entity"></param>
        ...
        // POST: Api/BillDelivery/SelectExportExcel/10
        /// <summary>
        ///     选择导出
        /// </summary>
        /// <param name="entity"></param>
        ...
        public async Task<AdminUiCallBack> SelectExportExcel([FromBody] FMArrayStringIds entity)
        {
            ...
            //获取list数据
            var listmodel =
                await _BillDeliveryServices.QueryListByClauseAsync(p => entity.id.Contains(p.deliveryId),
            ...
            var fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "-BillDelivery导出(选择结果).xls";
        ...
        // POST: Api/BillDelivery/QueryExportExcel/10
        /// <summary>
        ///     查询导出
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<AdminUiCallBack> QueryExportExcel()
        {
            ...
            //获取list数据
            var listmodel =
                await _BillDeliveryServices.QueryListByClauseAsync(where, p => p.deliveryId,
                    OrderByType.Asc);
            //给sheet1添加第一行的头部标题
            var row1 = sheet1.CreateRow(0);
            row1.CreateCell(0).SetCellValue("发货单序列");
            ...
            var webRootPath = _webHostEnvironment.WebRootPath;
            var tpath = "/files/" + DateTime.Now.ToString("yyyy-MM-dd") + "/";
            var fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "-BillDelivery导出(查询结果).xls";
            var filePath = webRootPath + tpath;
            var di = new DirectoryInfo(filePath);
            if (!di.Exists) di.Create();
            var fileHssf = new FileStream(filePath + fileName, FileMode.Create);
