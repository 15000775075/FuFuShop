...
namespace FuFuShop.Admin.Controllers
{
    ...
    public class BillReshipController : ControllerBase
    {
        ...
        // POST: Api/BillReship/GetPageList
        /// <summary>
        ///     获取列表
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<AdminUiCallBack> GetPageList()
        {
            ...
            var where = PredicateBuilder.True<BillReship>();
            //获取排序字段
            var orderField = Request.Form["orderField"].FirstOrDefault();
            Expression<Func<BillReship, object>> orderEx;
            switch (orderField)
            {
                case "reshipId":
            ...
            //获取数据
            var list = await _BillReshipServices.QueryPageAsync(where, orderEx, orderBy, pageCurrent, pageSize);
            //返回数据
            jm.data = list;
            jm.code = 0;
        ...
        // POST: Api/BillReship/DoDelete/10
        /// <summary>
        ///     确认收货
        /// </summary>
        ...
        public async Task<AdminUiCallBack> DoAudit([FromBody] FMStringId entity)
        {
            ...
            var model = await _BillReshipServices.QueryByIdAsync(entity.id);
            if (model == null)
            {
            ...
            var bl = await _BillReshipServices.UpdateAsync(
                p => new BillReship
        ...
        // POST: Api/BillReship/GetDetails/10
        /// <summary>
        ///     预览数据
        /// </summary>
        /// <param name="entity"></param>
        ...
        public async Task<AdminUiCallBack> GetDetails([FromBody] FMStringId entity)
        {
            ...
            var model = await _BillReshipServices.GetDetails(p => p.reshipId == entity.id, p => p.createTime, OrderByType.Desc);
            if (model == null)
            {
                jm.msg = GlobalConstVars.DataisNo;
                return jm;
        ...
        // POST: Api/BillReship/SelectExportExcel/10
        /// <summary>
        ///     选择导出
        /// </summary>
        /// <param name="entity"></param>
        ...
        public async Task<AdminUiCallBack> SelectExportExcel([FromBody] FMArrayStringIds entity)
        {
            ...
            //获取list数据
            var listmodel =
                await _BillReshipServices.QueryListByClauseAsync(p => entity.id.Contains(p.reshipId),
            ...
            var fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "-BillReship导出(选择结果).xls";
        ...
        // POST: Api/BillReship/QueryExportExcel/10
        /// <summary>
        ///     查询导出
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<AdminUiCallBack> QueryExportExcel()
        {
            ...
            //获取list数据
            var listmodel =
                await _BillReshipServices.QueryListByClauseAsync(where, p => p.reshipId, OrderByType.Asc);
            //给sheet1添加第一行的头部标题
            var row1 = sheet1.CreateRow(0);
            row1.CreateCell(0).SetCellValue("退货单号");
            row1.CreateCell(1).SetCellValue("订单序列");
            ...
            var webRootPath = _webHostEnvironment.WebRootPath;
            var tpath = "/files/" + DateTime.Now.ToString("yyyy-MM-dd") + "/";
            var fileName = DateTime.Now.ToString("yyyyMMddHHmmssfff") + "-BillReship导出(查询结果).xls";
            var filePath = webRootPath + tpath;
            var di = new DirectoryInfo(filePath);
            if (!di.Exists) di.Create();
            var fileHssf = new FileStream(filePath + fileName, FileMode.Create);
