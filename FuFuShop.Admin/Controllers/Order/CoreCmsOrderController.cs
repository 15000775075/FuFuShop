...
using FuFuShop.Model.ViewModels.Excel;
...
namespace FuFuShop.Admin.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        #region 选择导出============================================================
        // POST: Api/Order/SelectExportExcel/10
        /// <summary>
        /// 选择导出
        /// </summary>
        /// <param name="entity"></param>
        ...
        public async Task<AdminUiCallBack> SelectExportExcel([FromBody] FMArrayStringIds entity)
        {
            ...
            //获取数据
            var list = await _OrderServices.QueryListAsync(p => entity.id.Contains(p.orderId), p => p.createTime, OrderByType.Desc);
            if (list != null && list.Any())
            {
                var areaCache = await _areaServices.GetCaChe();
                foreach (var item in list)
                {
                    //item.operating = _OrderServices.GetOperating(item.orderId, item.status, item.payStatus, item.shipStatus, item.isdel);
                    //item.afterSaleStatus = "";
                    //if (item.aftersalesItem != null && item.aftersalesItem.Any())
                    //{
                    //    foreach (var sale in item.aftersalesItem)
        ...
        #region 查询导出============================================================
        // POST: Api/Order/QueryExportExcel/10
        /// <summary>
        /// 查询导出
        /// </summary>
        /// <returns></returns>
        ...
        public async Task<AdminUiCallBack> QueryExportExcel()
        {
            ...
            //获取数据
            var list = await _OrderServices.QueryListAsync(where, p => p.createTime, OrderByType.Desc);
            if (list != null && list.Any())
            {
                var areaCache = await _areaServices.GetCaChe();
                foreach (var item in list)
        ...
        #region 设置是否开发票============================================================
        // POST: Api/Order/DoSettaxType/10
        /// <summary>
        /// 设置是否开发票
        /// </summary>
        /// <param name="entity"></param>
