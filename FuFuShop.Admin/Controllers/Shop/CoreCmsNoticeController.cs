...
namespace FuFuShop.Admin.Controllers
{
    ...
    public class NoticeController : ControllerBase
    {
        ...
        public NoticeController(IWebHostEnvironment webHostEnvironment,
        ...
        public async Task<AdminUiCallBack> GetPageList()
        {
            ...
            var where = PredicateBuilder.True<Notice>();
            //获取排序字段
            var orderField = Request.Form["orderField"].FirstOrDefault();
            Expression<Func<Notice, object>> orderEx;
            switch (orderField)
            {
                case "id":
                    orderEx = p => p.id;
            ...
            //获取数据
            var list = await _NoticeServices.QueryPageAsync(where, orderEx, orderBy, pageCurrent, pageSize);
            //返回数据
            jm.data = list;
            jm.code = 0;
        ...
        [HttpPost]
        [Description("创建提交")]
        public async Task<AdminUiCallBack> DoCreate([FromBody] Notice entity)
        {
            var jm = new AdminUiCallBack();
            ...
            var bl = await _NoticeServices.InsertAsync(entity) > 0;
            jm.code = bl ? 0 : 1;
        ...
        [HttpPost]
        [Description("编辑提交")]
        public async Task<AdminUiCallBack> DoEdit([FromBody] Notice entity)
        {
            var jm = new AdminUiCallBack();

            var oldModel = await _NoticeServices.QueryByIdAsync(entity.id);
            if (oldModel == null)
            {
                jm.msg = "不存在此信息";
                return jm;
            ...
            //事物处理过程结束
            var bl = await _NoticeServices.UpdateAsync(oldModel);
            jm.code = bl ? 0 : 1;
        ...
        public async Task<AdminUiCallBack> DoDelete([FromBody] FMIntId entity)
        {
            ...
            var bl = await _NoticeServices.DeleteByIdAsync(entity.id);
            jm.code = bl ? 0 : 1;
        ...
        // POST: Api/Notice/DoBatchDelete/10,11,20
        /// <summary>
        ///     批量删除
        /// </summary>
        /// <param name="entity"></param>
        ...
        public async Task<AdminUiCallBack> DoBatchDelete([FromBody] FMArrayIntIds entity)
        {
            ...
            var bl = await _NoticeServices.DeleteByIdsAsync(entity.id);
            jm.code = bl ? 0 : 1;
            jm.msg = bl ? GlobalConstVars.DeleteSuccess : GlobalConstVars.DeleteFailure;
