...
namespace FuFuShop.Admin.Controllers
{
    ...
    public class CoreCmsStockLogController : ControllerBase
    {
        ...
        public async Task<AdminUiCallBack> GetPageList()
        {
            ...
            var where = PredicateBuilder.True<StockLog>();
            //获取排序字段
            var orderField = Request.Form["orderField"].FirstOrDefault();

            Expression<Func<StockLog, object>> orderEx = orderField switch
            {
                "id" => p => p.id,
                "stockId" => p => p.stockId,
            ...
            //获取数据
            var list = await _StockLogServices.QueryPageAsync(where, orderEx, orderBy, pageCurrent, pageSize, true);
            //返回数据
            jm.data = list;
            jm.code = 0;
