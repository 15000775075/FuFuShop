...
namespace FuFuShop.Repository
{
    ...
    /// 用户对表的提交记录 接口实现
    /// </summary>
    public class FormSubmitRepository : BaseRepository<FormSubmit>, IFormSubmitRepository
    {
        public FormSubmitRepository(IUnitOfWork unitOfWork) : base(unitOfWork)
        {
        }
        ...
        public new async Task<AdminUiCallBack> UpdateAsync(FormSubmit entity)
        {
            ...
            var oldModel = await DbClient.Queryable<FormSubmit>().In(entity.id).SingleAsync();
            if (oldModel == null)
            {
                jm.msg = "不存在此信息";
                return jm;
        ...
        public new async Task<AdminUiCallBack> DeleteByIdAsync(object id)
        {
            ...
            var bl = await DbClient.Deleteable<FormSubmit>(id).ExecuteCommandHasChangeAsync();
            jm.code = bl ? 0 : 1;
        ...
        public new async Task<AdminUiCallBack> DeleteByIdsAsync(int[] ids)
        {
            ...
            var bl = await DbClient.Deleteable<FormSubmit>().In(ids).ExecuteCommandHasChangeAsync();
            jm.code = bl ? 0 : 1;
            jm.msg = bl ? GlobalConstVars.DeleteSuccess : GlobalConstVars.DeleteFailure;
            ...
            int pageSize = 20, bool blUseNoLock = false)
        {
            ...
            List<FormSubmit> page;
            if (blUseNoLock)
            {
                page = await DbClient.Queryable<FormSubmit, FuFuShopUser>((p, sc) => new JoinQueryInfos(
                        JoinType.Left, p.userId == sc.id))
                        .Select((p, sc) => new FormSubmit
                        {
                            id = p.id,
                            formId = p.formId,
                            formName = p.formName,
            ...
            var list = new PageList<FormSubmit>(page, pageIndex, pageSize, totalCount);
            return list;
        }
        ...
        public async Task<WebApiCallBack> Pay(int id)
        {
            ...
            await DbClient.Updateable<FormSubmit>().SetColumns(p => p.payStatus == true).Where(p => p.id == id && p.payStatus == false)
                .ExecuteCommandAsync();
            jm.status = true;
            jm.data = jm.msg = "支付成功";
        ...
        public async Task<FormStatisticsViewDto> GetStatisticsByFormid(int formId, int day)
        {
            ...
            var list = await DbClient.Queryable<FormSubmit>()
