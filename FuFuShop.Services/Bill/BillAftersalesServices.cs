...
using FuFuShop.Model.ViewModels.QueryMuch;
...
namespace FuFuShop.Services
{
    ...
    /// 退货单表 接口实现
    /// </summary>
    public class BillAftersalesServices : BaseServices<BillAftersales>, IBillAftersalesServices
    {
        private readonly IBillAftersalesRepository _dal;
        ...
        public WebApiCallBack OrderToAftersales(string orderId, int aftersaleLevel = 0)
        {
            ...
            //算退货商品明细
            var list = base.QueryMuch<BillAftersalesItem, BillAftersales, QMAftersalesItems>(
        ...
        public async Task<WebApiCallBack> ToAdd(int userId, string orderId, int type, JArray items, string[] images, string reason, decimal refund)
        {
            ...
            var imagesServices = container.ServiceProvider.GetService<IBillAftersalesImagesServices>();
            var itemServices = container.ServiceProvider.GetService<IBillAftersalesItemServices>();
            ...
            orderInfo = result.data as Order;
            ...
            var billAftersales = new BillAftersales();
            ...
            if (items != null && items.Any())
            {
                ...
                {
                    var list = aftersalesItems.data as List<BillAftersalesItem>;
            ...
            if (images.Length > 0)
            {
                var imagesList = new List<BillAftersalesImages>();
                for (int i = 0; i < images.Length; i++)
                {
                    imagesList.Add(new BillAftersalesImages()
                    {
        ...
        /// <returns></returns>
        private WebApiCallBack Verify(int type, Order orderInfo, decimal refund, JArray items)
        {
            var jm = new WebApiCallBack();
            //判断订单是否是可以售后
            //只有活动订单才能售后
        ...
        /// <param name="items"></param>
        /// <returns></returns>
        private WebApiCallBack verifyNums(int type, Order orderInfo, JArray items)
        {
            var jm = new WebApiCallBack();
        ...
        /// <returns></returns>
        private WebApiCallBack formatAftersalesItems(Order orderInfo, JArray items, string aftersalesId)
        {
            var jm = new WebApiCallBack();
            var data = new List<BillAftersalesItem>();
            foreach (var item in items)
            {
                var id = ((JObject)item)["id"].ObjectToInt();
                var nums = ((JObject)item)["nums"].ObjectToInt();
                ...
                foreach (var orderItem in orderInfo.items)
                {
                    if (orderItem.id == id)
                    {
                        ...
                        var billAftersalesItem = new BillAftersalesItem
                        {
                            aftersalesId = aftersalesId,
        ...
        /// <param name="userId"></param>
        /// <returns></returns>
        public async Task<BillAftersales> GetInfo(string aftersalesId, int userId)
        {
            ...
            JArray items)
        {
            ...
            var info = res.data as BillAftersales;
            ...
            //如果是退货，判断退货明细，数量是否超出可退的数量
            var billAftersalesItems = new List<BillAftersalesItem>();
            if (items.Count > 0)
            {
                var aftersalesItems = formatAftersalesItems(orderInfo, items, aftersalesId);
                if (!aftersalesItems.status)
                ...
                billAftersalesItems = aftersalesItems.data as List<BillAftersalesItem>;
            }
            ...
            using (var container = _serviceProvider.CreateScope())
            {
                var itemServices = container.ServiceProvider.GetService<IBillAftersalesItemServices>();
                var orderServices = container.ServiceProvider.GetService<IOrderServices>();
                var refundServices = container.ServiceProvider.GetService<IBillRefundServices>();
                var reshipServices = container.ServiceProvider.GetService<IBillReshipServices>();
                var goodsServices = container.ServiceProvider.GetService<IGoodsServices>();
                //更新售后单
                await _dal.UpdateAsync(p => new BillAftersales() { status = status, mark = mark, refundAmount = refund, type = type },
                ...
                if (status == (int)GlobalEnumVars.BillAftersalesStatus.Success)
                {
                    ...
                    await orderServices.UpdateAsync(
                        p => new Order() { status = orderInfo.status, payStatus = orderInfo.payStatus },
        ...
        public async Task<WebApiCallBack> PreAudit(string aftersalesId)
        {
            ...
            {
                model.order = orderResult.data as Order;
