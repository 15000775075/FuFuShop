...
namespace FuFuShop.Services.Bill
{
    ...
    public class BillDeliveryServices : BaseServices<BillDelivery>, IBillDeliveryServices
    {
        ...
        public async Task<WebApiCallBack> BatchShip(string[] orderId, string logiCode, string logiNo, Dictionary<int, int> items, int storeId = 0, string shipName = "", string shipMobile = "", int shipAreaId = 0, string shipAddress = "", string memo = "")
        {
            ...
            var orderService = container.ServiceProvider.GetService<IOrderServices>();
            var stockServices = container.ServiceProvider.GetService<IStockServices>();
            //获取订单详情
            ...
            //设置发货明细
            var bdRel = new List<BillDeliveryItem>();
            ...
            foreach (var item in items)
            {
                ...
                //构建发货单明细
                var bdItem = new BillDeliveryItem();
        ...
        private async Task<bool> OrderShip(Order orderInfo, Dictionary<int, int> items, BillDelivery deliveryInfo, int storeId = 0)
        {
            using var container = _serviceProvider.CreateScope();
            var orderService = container.ServiceProvider.GetService<IOrderServices>();
            var billLadingService = container.ServiceProvider.GetService<IBillLadingServices>();
            var messageCenterServices = container.ServiceProvider.GetService<IMessageCenterServices>();
            var logisticsServices = container.ServiceProvider.GetService<ILogisticsServices>();
            var stockLogServices = container.ServiceProvider.GetService<IStockLogServices>();
            var stockServices = container.ServiceProvider.GetService<IStockServices>();
