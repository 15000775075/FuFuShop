...
namespace FuFuShop.Services
{
    ...
    /// 支付单表 接口实现
    /// </summary>
    public class BillPaymentsServices : BaseServices<BillPayments>, IBillPaymentsServices
    {
        private readonly IBillPaymentsRepository _dal;
        private readonly IUnitOfWork _unitOfWork;
        private readonly IServiceProvider _serviceProvider;


        private readonly ISettingServices _settingServices;
        private readonly IFormSubmitServices _formSubmitServices;
        ...
        public BillPaymentsServices(IUnitOfWork unitOfWork
            , IBillPaymentsRepository dal
            , ISettingServices settingServices
            , IHttpContextAccessor httpContextAccessor
            , IFormSubmitServices formSubmitServices
        ...
        public async Task<WebApiCallBack> Pay(string sourceStr, string paymentCode, int userId, int type, JObject @params)
        {
            ...
            var billPayments = result.data as BillPayments;
            //根据支付方式返回支付配置
            //微信支付
        ...
        private async Task<WebApiCallBack> CheckOpenId(string paymentCode, JObject jobj)
        {
            ...
            if (jobj != null)
            {
                ...
                jm.msg = $"https://open.weixin.qq.com/connect/oauth2/authorize?appid={_weChatOptions.WeiXinAppId}&redirect_uri={redirectUrl}&response_type={"code"}&scope={3}&state={""}{"&connect_redirect=1"}#wechat_redirect";
            }
            return jm;
        }
        ...
        private async Task<WebApiCallBack> ToAdd(string sourceStr, string paymentCode, int userId = 0, int type = (int)GlobalEnumVars.BillPaymentsType.Order, JObject @params = null)
        {
            ...
            var billPayments = new BillPayments();
        ...
        #region 扩展方法
        private async Task<string> PayTitle(BillPayments entity)
        {
        ...
        public async Task<WebApiCallBack> ToPay(string orderId, int type, string paymentCode)
        {
            using (var container = _serviceProvider.CreateScope())
            {
                ...
                var data = result.data as BillPayments;
                //支付单支付
