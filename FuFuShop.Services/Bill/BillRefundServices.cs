...
namespace FuFuShop.Services
{
    ...
    /// 退款单表 接口实现
    /// </summary>
    public class BillRefundServices : BaseServices<BillRefund>, IBillRefundServices
    {
        private readonly IBillRefundRepository _dal;
        ...
        public BillRefundServices(IUnitOfWork unitOfWork,
            IBillRefundRepository dal,
        ...
        public async Task<WebApiCallBack> ToRefund(string refundId, int status, string paymentCodeStr = "")
        {
            ...
            if (status == (int)GlobalEnumVars.BillRefundStatus.STATUS_REFUND)
            {
                ...
                else
                {
                    ...
                    await _dal.UpdateAsync(p => new BillRefund()
                    {
        ...
        public async Task<WebApiCallBack> PaymentRefund(string refundId)
        {
            ...
            if (jm.status)
            {
                ...
                await _dal.UpdateAsync(p => new BillRefund() { status = (int)GlobalEnumVars.BillRefundStatus.STATUS_REFUND, memo = res }, p => p.refundId == refundId);
            }
            else
            {
                var res = JsonConvert.SerializeObject(jm.data);
                await _dal.UpdateAsync(p => new BillRefund() { status = (int)GlobalEnumVars.BillRefundStatus.STATUS_FAIL, memo = res }, p => p.refundId == refundId);
            }
