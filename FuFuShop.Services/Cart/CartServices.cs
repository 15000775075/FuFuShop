...
namespace FuFuShop.Services
{
    ...
    public class CartServices : BaseServices<Cart>, ICartServices
    {
        ...
        public async Task<WebApiCallBack> Add(int userId, int productId, int nums, int numType, int cartTypes = 1, int objectId = 0)
        {
            ...
            switch (cartTypes)
            {
                case (int)GlobalEnumVarsType.Common:

                    break;
                case (int)GlobalEnumVarsType.PinTuan:
                    numType = (int)GlobalEnumVarsType.PinTuan;
                    //拼团模式去判断是否开启拼团，是否存在
                    ...
                    await _dal.DeleteAsync(p => p.type == (int)GlobalEnumVarsType.PinTuan && p.userId == userId);
                    catInfo = null;
                    break;
                case (int)GlobalEnumVarsType.Group or (int)GlobalEnumVarsType.Skill:
                    //标准模式不需要做什么判断
                    //判断商品是否做团购秒杀
                    if (goodsServices.IsInGroup((int)products.goodsId, out var promotionsModel, objectId) == true)
                    {
                        ...
                        var typeIds = new int[] { (int)GlobalEnumVarsType.Group, (int)GlobalEnumVarsType.Skill };
                        //此人的购物车中的所有购物车拼团商品都删掉，因为立即购买也是要加入购物车的，所以需要清空之前历史的加入过购物车的商品
                    ...
                    break;
                case (int)GlobalEnumVarsType.Bargain:

                    break;

                case (int)GlobalEnumVarsType.Solitaire:
        ...
        public async Task<WebApiCallBack> AddCartHavePinTuan(int productId, int userId = 0, int nums = 1, int teamId = 0)
        {
            ...
            using var container = _serviceProvider.CreateScope();
            var orderRepository = container.ServiceProvider.GetService<IOrderRepository>();
            var checkOrder = orderRepository.FindLimitOrder(products.id, userId, pinTuanRule.startTime, pinTuanRule.endTime, (int)GlobalEnumVarsType.PinTuan);
        ...
        public async Task<WebApiCallBack> GetCartDtoData(int userId, int[] ids = null, int type = 1, int objectId = 0)
        {
            ...
            switch (type)
            {
                ...
                case (int)GlobalEnumVarsType.PinTuan:
                    ...
                    break;
                case (int)GlobalEnumVarsType.Group:
                    //团购模式不需要修改订单数据和商品数据
                    break;
                case (int)GlobalEnumVarsType.Skill:
                    //秒杀模式不需要修改订单数据和商品数据
                    break;
                case (int)GlobalEnumVarsType.Bargain:
                    //砍价模式
        ...
        /// <returns></returns>
        public async Task<WebApiCallBack> GetCartInfos(int userId, int[] ids, int orderType, int areaId, int point, string couponCode, bool freeFreight = false, int deliveryType = (int)GlobalEnumVarsReceiptType.Logistics, int objectId = 0)
        {
            ...
            //门店订单，强制无运费
            if (deliveryType == (int)GlobalEnumVarsReceiptType.IntraCityService || deliveryType == (int)GlobalEnumVarsReceiptType.SelfDelivery)
            {
                freeFreight = true;
            }
            //运费判断
            ...
            if (orderType == (int)GlobalEnumVarsType.Common)
            {
                jm.data = await _promotionServices.ToPromotion(cartDto);
                jm.msg = "订单促销金额计算";
            }
            else if ((orderType == (int)GlobalEnumVarsType.Group || orderType == (int)GlobalEnumVarsType.Skill) && objectId > 0)
            {
                //团购秒杀默认时间过期后，不可以下单
                var dt = DateTime.Now;
            ...
            else if (orderType == (int)GlobalEnumVarsType.PinTuan)
            {
                jm.data = await _promotionServices.ToPromotion(cartDto);
                jm.msg = "拼团也计算促销信息";
            }
        ...
        public async Task<WebApiCallBack> CartPoint(CartDto cartDto, int userId, int point)
        {
            ...
            if (point != 0)
            {
                ...
                var ordersPointProportion = CommonHelper.GetConfigDictionary(allConfigs, SystemSettingConstVarssPointProportion).ObjectToInt(10);//订单积分使用比例
        ...
        public async Task<WebApiCallBack> GetCartAvailableCoupon(int userId, int[] ids = null)
        {
            ...
            //获取货品数据
            var carts = await _dal.QueryListByClauseAsync(p => p.userId == userId && p.type == (int)GlobalEnumVarsType.Common);
            if (!carts.Any())
            {
                return jm;
            }
