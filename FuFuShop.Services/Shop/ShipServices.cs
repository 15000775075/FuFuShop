...
namespace FuFuShop.Services.Shop
{
    ...
    /// 配送方式表 接口实现
    /// </summary>
    public class ShipServices : BaseServices<Ship>, IShipServices
    {
        private readonly IShipRepository _dal;
        private readonly IUnitOfWork _unitOfWork;
        public ShipServices(IUnitOfWork unitOfWork, IShipRepository dal)
        {
            _dal = dal;
            BaseDal = dal;
        ...
        public decimal GetShipCost(int areaId = 0, decimal weight = 0, decimal totalmoney = 0)
        {
            ...
            //先判断是否子地区满足条件
            var def = QueryByClause(p =>
                 p.status == (int)GlobalEnumVars.ShipStatus.Yes &&
                 p.areaType == (int)GlobalEnumVars.ShipAreaType.Part && p.areaFee.Contains(idStr));
            //没有子地区取默认
            if (def == null)
            {
                def = QueryByClause(p => p.isDefault == true && p.status == (int)GlobalEnumVars.ShipStatus.Yes);
            }
            //没有默认取启用状态
            if (def == null)
            {
                def = QueryByClause(p => p.status == (int)GlobalEnumVars.ShipStatus.Yes);
                if (def == null)
                {//没有配送方式，返回0
                    return postfee;
                }
        ...
        /// <returns></returns>
        public decimal calculate_fee(Ship ship, decimal weight, decimal totalmoney = 0, decimal firstunitAreaPrice = 0)
        {
            //满多少免运费
        ...
        /// 根据地区获取配送方式
        /// </summary>
        public Ship GetShip(int areaId = 0)
        {
            var idStr = areaId.ToString();
            //先判断是否子地区满足条件
            var def = QueryByClause(p =>
