...
namespace FuFuShop.Services
{
    ...
    /// 用户余额表 接口实现
    /// </summary>
    public class UserBalanceServices : BaseServices<UserBalance>, IBalanceServices
    {
        private readonly IUserBalanceRepository _dal;
        private readonly IUnitOfWork _unitOfWork;
        private readonly IServiceProvider _serviceProvider;

        public UserBalanceServices(IUnitOfWork unitOfWork, IUserBalanceRepository dal,
        ...
        public async Task<WebApiCallBack> Change(int userId, int type, decimal money, string sourceId = "", decimal cateMoney = 0)
        {
            ...
            if (money != 0)
            {
                ...
                var balanceModel = new UserBalance();
                ...
                //更新用户数据
                await userServices.UpdateAsync(p => new FuFuShopUser() { balance = balance }, p => p.id == userId);
        ...
        /// <returns></returns>
        public new async Task<IPageList<UserBalance>> QueryPageAsync(Expression<Func<UserBalance, bool>> predicate,
