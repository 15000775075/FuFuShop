...
namespace FuFuShop.Services
{
    ...
    /// 银行卡信息 接口实现
    /// </summary>
    public class UserBankCardServices : BaseServices<UserBankCard>, IUserBankCardServices
    {
        private readonly IUserBankCardRepository _dal;
        private readonly IAreaServices _areaServices;
        private readonly ISettingServices _settingServices;


        private readonly IUnitOfWork _unitOfWork;
        public UserBankCardServices(IUnitOfWork unitOfWork, IUserBankCardRepository dal, IAreaServices areaServices, ISettingServices settingServices)
        {
            _dal = dal;
            base.BaseDal = dal;
        ...
        /// <returns></returns>
        public async Task<WebApiCallBack> AddBankCards(UserBankCard entity)
        {
            ...
            if (model.isdefault == true)
            {
                ...
                if (def)
                {
                    ...
                    await _dal.UpdateAsync(p => new UserBankCard() { isdefault = false }, p => p.isdefault == true && p.userId == entity.userId);
        ...
        public async Task<WebApiCallBack> SetDefault(int userId, int id)
        {
            ...
            if (card != null)
            {
                ...
                if (defCard != null)
                {
                    await _dal.UpdateAsync(it => new UserBankCard() { isdefault = false },
                        p => p.id == defCard.id);
                    await _dal.UpdateAsync(it => new UserBankCard() { isdefault = true },
                        p => p.id == card.id);
                    jm.status = true;
                    jm.msg = "保存成功";
                }
