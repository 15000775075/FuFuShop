...
namespace FuFuShop.Services
{
    ...
    /// 用户积分记录表 接口实现
    /// </summary>
    public class UserPointLogServices : BaseServices<UserPointLog>, IUserPointLogServices
    {
        private readonly IUserPointLogRepository _dal;
        private readonly IUnitOfWork _unitOfWork;
        private readonly IServiceProvider _serviceProvider;

        public UserPointLogServices(IUnitOfWork unitOfWork, IUserPointLogRepository dal,
        ...
        public async Task<WebApiCallBack> SetPoint(int userId, int num, int type, string remarks)
        {
            using (var container = _serviceProvider.CreateScope())
            {
                ...
                await userServices.UpdateAsync(p => new FuFuShopUser() { point = p.point + num }, p => p.id == userModel.id);
        ...
        public async Task OrderComplete(int userId, decimal money, string orderId)
        {
            using (var container = _serviceProvider.CreateScope())
            {
                var _settingServices = container.ServiceProvider.GetService<ISettingServices>();
        ...
        public async Task<WebApiCallBack> IsSign(int userId)
        {
            ...
            var where = PredicateBuilder.True<UserPointLog>();
            ...
            using (var container = _serviceProvider.CreateScope())
            {
                ...
                else
                {
                    ...
                    var logs = await _dal.QueryListByClauseAsync(p =>
                         p.userId == userId && p.type == (int)GlobalEnumVars.UserPointSourceTypes.PointTypeSign &&
                         p.createTime > day);
