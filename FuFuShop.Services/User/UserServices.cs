...
namespace FuFuShop.Services
{
    ...
    public class UserServices : BaseServices<FuFuShopUser>, IUserServices
    {
        ...
        private readonly IBalanceServices _userBalanceServices;
        ...
        public UserServices(IUnitOfWork unitOfWork
            , IUserRepository dal
            , IBalanceServices userBalanceServices
            , ISettingServices settingServices
            , IUserPointLogServices userPointLogServices, IUserWeChatInfoServices userWeChatInfoServices, IUserGradeServices userGradeServices, PermissionRequirement permissionRequirement, IHttpContextAccessor httpContextAccessor, IUserLogServices userLogServices)
        {
            _dal = dal;
            base.BaseDal = dal;
        ...
        public async Task<AdminUiCallBack> UpdateBalance(int id, decimal money)
        {
            ...
            var up = await _dal.UpdateAsync(p => new FuFuShopUser() { balance = newMoney }, p => p.id == id);
            if (up)
            {
        ...
        public async Task<AdminUiCallBack> UpdatePiont(FMUpdateUserPoint entity)
        {
            ...
            var up = await _dal.UpdateAsync(p => new FuFuShopUser() { point = newPoint }, p => p.id == entity.id);
            if (up)
            {
                var point = new UserPointLog();
        ...
        public async Task<WebApiCallBack> ChangePassword(int userId, string newPwd, string password = "")
        {
            ...
            var bl = await _dal.UpdateAsync(p => new FuFuShopUser() { passWord = md5Pwd, updataTime = DateTime.Now }, p => p.id == userId);
            jm.status = bl;
        ...
        public async Task<WebApiCallBack> SetMyInvite(int superiorId, int userId)
        {
            ...
            var bl = await _dal.UpdateAsync(p => new FuFuShopUser() { parentId = superiorId }, p => p.id == userId);
            jm.status = bl;
        ...
        private async Task<WebApiCallBack> EditPwd(int userId, string newPwd, DateTime createTime)
        {
            ...
            var up = await _dal.UpdateAsync(p => new FuFuShopUser() { passWord = md5Pwd }, p => p.id == userId);
            if (!up)
            {
                jm.status = false;
                jm.msg = "密码修改失败";
        ...
        public async Task<WebApiCallBack> SmsLogin(FMWxAccountCreate entity, int loginType = (int)GlobalEnumVars.LoginType.WeChatPhoneNumber, int platform = 1)
        {
            ...
            if (!string.IsNullOrEmpty(entity.sessionAuthId))
            {
                var updateAsync = await _userWeChatInfoServices.UpdateAsync(p => new UserWeChatInfo() { userId = userInfo.id }, p => p.openid == entity.sessionAuthId);
                if (updateAsync)
                {
                    //多个微信可能同时授权一个号码登录。
                    ...
                    //会导致微信数据报错（）
                    await _userWeChatInfoServices.UpdateAsync(p => new UserWeChatInfo() { userId = 0 }, p => p.openid != entity.sessionAuthId && p.userId == userInfo.id);
                }
                //如果是别的未绑定微信用户进来，则反向直接关联。
                var wxUserInfo = await _userWeChatInfoServices.QueryByClauseAsync(p => p.openid == entity.sessionAuthId);
                if (wxUserInfo != null)
                {
                    await _dal.UpdateAsync(p => new FuFuShopUser() { userWx = wxUserInfo.id }, p => p.id == userInfo.id);
                }
            }
        ...
        public Task<WebApiCallBack> ForgetPassword(string mobile, string code, string newPwd)
        {
            throw new NotImplementedException();
        }

        Task<IPageList<User>> IUserServices.QueryPageAsync(Expression<Func<FuFuShopUser, bool>> predicate, Expression<Func<FuFuShopUser, object>> orderByExpression, OrderByType orderByType, int pageIndex, int pageSize)
        {
            throw new NotImplementedException();
        }
    }
