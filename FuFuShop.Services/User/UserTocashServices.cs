...
namespace FuFuShop.Services
{
    ...
    /// 用户提现记录表 接口实现
    /// </summary>
    public class UserTocashServices : BaseServices<UserTocash>, IUserTocashServices
    {
        private readonly IUserTocashRepository _dal;
        ...
        public UserTocashServices(IUnitOfWork unitOfWork, IUserTocashRepository dal,
            IServiceProvider serviceProvider, ISettingServices settingServices, IUserServices userServices, IBalanceServices userBalanceServices)
        {
            _dal = dal;
            base.BaseDal = dal;
        ...
        public async Task<WebApiCallBack> Tocash(int userId, decimal money, int bankCardsId)
        {
            ...
            var userBankCardServices = container.ServiceProvider.GetService<IUserBankCardServices>();
            var balanceServices = container.ServiceProvider.GetService<IBalanceServices>();
        ...
        public async Task<WebApiCallBack> UserToCashList(int userId = 0, int page = 1, int limit = 10, int status = 0)
        {
            ...
            var where = PredicateBuilder.True<UserTocash>();
            if (status > 0)
            {
        ...
        public async Task<WebApiCallBack> Examine(int id = 0, int status = 0)
        {
            ...
            if (status > 0)
            {
                var bl = await _dal.UpdateAsync(p => new UserTocash() { status = status, updateTime = DateTime.Now }, p =>
                         p.id == id && p.status == (int)GlobalEnumVars.UserTocashTypes.待审核);
                jm.status = bl;
                jm.data = status;
                if (bl)
                {
                    ...
                    if (status == (int)GlobalEnumVars.UserTocashTypes.提现失败)
                    {
                        ...
                        var up = await _userServices.UpdateAsync(p => new FuFuShopUser() { balance = p.balance + newMoney }, p => p.id == toCashInfo.userId);
                        if (up)
                        {
                            //添加记录
        ...
        /// <returns></returns>
        public new async Task<IPageList<UserTocash>> QueryPageAsync(Expression<Func<UserTocash, bool>> predicate,
